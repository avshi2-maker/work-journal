...
async function capOpenModal() {
  ...
  if (_snagFrames && _snagFrames.length && _snagFrames[0]) {
    var _capImgSrc = 'data:image/jpeg;base64,' + _snagFrames[0];
    capPhotoDiv.innerHTML = '<div style="font-size:11px;color:#666;margin-bottom:6px;">📸 תמונה שנסרקה — לחץ להגדלה:</div>'
      + '<img src="' + _capImgSrc + '" onclick="openLightbox(\'' + _capImgSrc + '\',\'תמונה שנסרקה\')" style="max-width:100%;max-height:200px;object-fit:contain;border-radius:8px;border:1px solid rgba(255,255,255,0.1);cursor:pointer;" title="לחץ להגדלה">';
