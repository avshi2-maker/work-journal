...
function encRenderGrouped(){
  ...
  function ib(icon,fn){
    // TASK1: speech bubble (💬 = &#128172;) is removed entirely — return empty string
    if(icon==='&#128172;')return '';
    // TASK1: eye (👁️ = &#128065;&#65039;) opens filter modal instead of default action
    // Extract the argument passed to encSetTypeTab/encSetSource/encToggleGroup and route to encRowFilter
    if(icon==='&#128065;&#65039;'){
      var m=fn.match(/\\?'([^']+)\\?'/);
      var arg=m?m[1]:'';
      return '<div style="'+iS+'" onclick="encRowFilter(\''+arg+'\')" title="סנן וצפה">'+icon+'</div>';
    }
    return '<div style="'+iS+'" onclick="'+fn+'">'+icon+'</div>';
  }
  ...
  var b1=box('#c9a84c','#fffbf0',
    ...
    '<div style="'+rowS+'">'+ib('&#128193;','encToggleGroup(\'general\')')+ib('&#128196;','encToggleGroup(\'general\')')+ib('&#128065;&#65039;','encToggleGroup(\'general\')')+bdg('finding','#fff3e0','#e65100','#ffcc80')+'<div style="'+txtS+'">&#9679; &#1502;&#1502;&#1510;&#1488;&#1497;&#1501; &#1492;&#1504;&#1491;&#1505;&#1497;&#1497;&#1501; &#8212; AI '+cn(cntFindings,'#fff3e0','#e65100')+'</div></div>'+
  ...
  var b2=box('#ef9a9a','#fff5f5',
    ...
    '<div style="'+rowS+'">'+ib('&#128193;','encToggleGroup(\'safety\')')+ib('&#128172;','encToggleGroup(\'safety\')')+ib('&#128196;','encToggleGroup(\'safety\')')+ib('&#128065;&#65039;','encToggleGroup(\'safety\')')+bdg('finding','#fff3e0','#e65100','#ffcc80')+'<div style="'+txtS+'">&#9679; &#1502;&#1502;&#1510;&#1488;&#1497;... '+cn(cntFindings,'#fff3e0','#e65100')+'</div></div>'+
  ...
  var b3=box('#4db6ac','#f0faf9',
    ...
    '<div style="'+rowS+'">'+ib('&#128193;','encSetTypeTab(\'image\')')+ib('&#128172;','encSetTypeTab(\'image\')')+ib('&#128065;&#65039;','encSetTypeTab(\'image\')')+bdg('image','#fce4ec','#880e4f','#f48fb1')+'<div style="'+txtS+'">&#9679; &#1514;&#1502;&#1493;&#1504;&#1493;&#1514; &#1513;&#1496;&#1495; &#1502;&#1488;&#1493;&#1513;&#1512;&#1493;&#1514; '+cn(cntImages,'#fce4ec','#880e4f')+'</div></div>'+
    '<div style="'+secS+'">&#1492;&#1511;&#1500;&#1496;&#1493;&#1514; &#1493;&#1513;&#1497;&#1495;&#1493;&#1514; (audio | call)</div>'+
    '<div style="'+rowS+'">'+ib('&#128193;','encSetTypeTab(\'audio\')')+ib('&#128172;','encSetTypeTab(\'audio\')')+ib('&#128065;&#65039;','encSetTypeTab(\'audio\')')+bdg('audio','#f3e5f5','#6a1b9a','#ce93d8')+'<div style="'+txtS+'">&#9679; &#1492;&#1511;&#1500;&#1496;&#1493;&#1514; &#1513;&#1496;&#1495; '+cn(cntAudio,'#f3e5f5','#6a1b9a')+'</div></div>'+
  ...
  var b4=box('#81c784','#f9fef9',
    ...
    '<div style="'+rowS+'">'+ib('&#128193;','encSetTypeTab(\'finding\')')+ib('&#128196;','encSetTypeTab(\'finding\')')+ib('&#128065;&#65039;','encSetTypeTab(\'finding\')')+bdg('finding','#fff3e0','#e65100','#ffcc80')+'<div style="'+txtS+'">&#9679; &#1504;&#1497;&#1514;&#1493;&#1495; &#1512;&#1493;&#1493;&#1495;/&#1492;&#1508;&#1505;&#1491; &#8212; AI '+cn(cntFindings,'#fff3e0','#e65100')+'</div></div>'+
  ...
  var b6=box('#ce93d8','#fdf8ff',
    ...
    '<div style="'+rowS+'">'+ib('&#128193;','encSetTypeTab(\'contacts\')')+ib('&#128172;','encSetTypeTab(\'contacts\')')+ib('&#128065;&#65039;','encSetTypeTab(\'contacts\')')+bdg('contacts','#f3e5f5','#4a148c','#ce93d8')+'<div style="'+txtS+'">&#9679; &#1505;&#1508;&#1511;&#1497;&#1501; &#1493;&#1511;&#1489;&#1500;&#1504;&#1497;&#1501; &#1500;&#1508;... '+cn(cntContacts,'#f3e5f5','#4a148c')+'</div></div>'+
...
// ══════════════════════════════════════════════════════
// TASK 1 — Row Filter Modal (eye 👁️ icon)
// Opens a filter panel with date range + project.
// After user filters, shows a list of matching items (title only).
// Clicking an item opens the correct preview (image/audio/video/pdf/text).
// ══════════════════════════════════════════════════════

function encRowFilter(rowType){
  // rowType examples: 'image','audio','call','pdf','finding','standard','price','takeoff','contacts','note','personal','__src_standards','__src_prices','__src_inbox','inbox','safety','general','archive','personal'
  var norm=String(rowType||'').replace('__src_','');
  // Collect all items that match this row type
  var pool=[];
  if(norm==='standards'||norm==='standard'){
    pool=_encItems.filter(function(i){return i._src==='standards';});
  } else if(norm==='prices'||norm==='price'){
    pool=_encItems.filter(function(i){return i._src==='prices';});
  } else if(norm==='takeoff'){
    pool=_encItems.filter(function(i){return i._src==='takeoff';});
  } else if(norm==='inbox'){
    pool=_encItems.filter(function(i){return i._src==='inbox';});
  } else if(norm==='contacts'){
    pool=_encContacts.slice();
  } else if(norm==='archive'){
    pool=_encArchive.slice();
  } else if(norm==='image'||norm==='audio'||norm==='video'||norm==='call'||norm==='pdf'){
    pool=_encItems.filter(function(i){return i._type===norm||(i.media_type||'').toLowerCase()===norm;});
  } else if(norm==='finding'){
    pool=_encItems.filter(function(i){return i._type==='finding';});
  } else if(norm==='note'){
    pool=_encItems.filter(function(i){return i._src==='notes'||i._type==='note';});
  } else if(norm==='personal'){
    pool=_encItems.filter(function(i){return i._type==='personal';});
  } else if(norm==='safety'){
    pool=_encItems.filter(function(i){var c=(i.category||'').toLowerCase();return c.includes('&#1489;&#1496;&#1497;&#1495;&#1493;&#1514;')||c.includes('safety');});
  } else if(norm==='general'||norm==='media'||norm==='legal'){
    pool=_encItems.slice();
  } else {
    // fallback: try _type then _src then category
    pool=_encItems.filter(function(i){
      return i._type===norm||i._src===norm||(i.category||'').toLowerCase()===norm;
    });
  }

  // Remove existing modal if any
  var existing=document.getElementById('enc-row-filter');
  if(existing)existing.remove();

  // Build project options from items in pool + allProjects
  var projSet={};
  ...
  var inpS=encInp();
  ov.innerHTML=
    '<div style="background:#fff;border-radius:14px;padding:20px;width:100%;max-width:640px;max-height:85vh;display:flex;flex-direction:column;direction:rtl;font-family:Heebo,sans-serif;">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">'+
        '<div>'+
          '<div style="font-size:10px;letter-spacing:2px;color:#9a6f00;font-weight:800;">FILTER &amp; PICK</div>'+
          '<div style="font-size:16px;font-weight:900;color:#1a3d5c;">&#128269; &#1505;&#1504;&#1503; &#1493;&#1489;&#1495;&#1512; &#1511;&#1493;&#1489;&#1509; '+encEsc(norm)+' ('+pool.length+')</div>'+
        '</div>'+
        '<button onclick="document.getElementById(\'enc-row-filter\').remove()" style="background:#f5f5f5;border:none;border-radius:50%;width:30px;height:30px;font-size:14px;cursor:pointer;">&#10005;</button>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;">'+
        '<div>'+
          '<div style="font-size:10px;font-weight:800;color:#666;margin-bottom:3px;">&#1502;&#1514;&#1488;&#1512;&#1497;&#1498;</div>'+
          '<input type="date" id="enc-rf-from" style="'+inpS+'">'+
        '</div>'+
        '<div>'+
          '<div style="font-size:10px;font-weight:800;color:#666;margin-bottom:3px;">&#1506;&#1491; &#1514;&#1488;&#1512;&#1497;&#1498;</div>'+
          '<input type="date" id="enc-rf-to" style="'+inpS+'">'+
        '</div>'+
        '<div style="grid-column:1/-1;">'+
          '<div style="font-size:10px;font-weight:800;color:#666;margin-bottom:3px;">&#1508;&#1512;&#1493;&#1497;&#1511;&#1496;</div>'+
          '<select id="enc-rf-proj" style="'+inpS+'">'+projOpts+'</select>'+
        '</div>'+
      '</div>'+
      '<div style="display:flex;gap:8px;margin-bottom:12px;">'+
        '<button onclick="encRowFilterApply(\''+norm+'\')" style="flex:1;padding:10px;background:#1a3d5c;border:none;color:#FFD700;border-radius:8px;font-family:Heebo,sans-serif;font-size:13px;font-weight:900;cursor:pointer;">&#128269; &#1505;&#1504;&#1503;</button>'+
        '<button onclick="encRowFilterClear(\''+norm+'\')" style="padding:10px 16px;background:#f5f5f5;border:none;border-radius:8px;font-family:Heebo,sans-serif;font-size:12px;cursor:pointer;">&#1504;&#1511;&#1492;</button>'+
      '</div>'+
      '<div id="enc-rf-results" style="flex:1;overflow-y:auto;border-top:1px solid #eee;padding-top:10px;">'+
        encRowFilterListHtml(pool,norm)+
      '</div>'+
    '</div>';

  // Stash pool on the modal element so Apply can re-filter it
  document.body.appendChild(ov);
  ov._pool=pool;
  ov._rowType=norm;
}

function encRowFilterApply(rowType){
  var ov=document.getElementById('enc-row-filter');if(!ov)return;
  var pool=ov._pool||[];
  var from=(document.getElementById('enc-rf-from')||{}).value||'';
  var to=(document.getElementById('enc-rf-to')||{}).value||'';
  var proj=(document.getElementById('enc-rf-proj')||{}).value||'';
  var fromT=from?new Date(from).getTime():0;
  var toT=to?new Date(to).getTime()+86400000:0; // include the "to" day
  var filtered=pool.filter(function(i){
    if(proj){var pid=i.source_project_id||i.project_id||'';if(pid!=proj)return false;}
    if(fromT||toT){
  ...
  var el=document.getElementById('enc-rf-results');
  if(el)el.innerHTML=encRowFilterListHtml(filtered,rowType);
}

function encRowFilterClear(rowType){
  var ov=document.getElementById('enc-row-filter');if(!ov)return;
  var f=document.getElementById('enc-rf-from');if(f)f.value='';
  var t=document.getElementById('enc-rf-to');if(t)t.value='';
  var p=document.getElementById('enc-rf-proj');if(p)p.value='';
  var el=document.getElementById('enc-rf-results');
  if(el)el.innerHTML=encRowFilterListHtml(ov._pool||[],rowType);
}

function encRowFilterListHtml(items,rowType){
  if(!items||!items.length){
    return '<div style="text-align:center;padding:30px 15px;color:#888;font-size:13px;">&#1488;&#1497;&#1503; &#1514;&#1493;&#1510;&#1488;&#1493;&#1514;</div>';
  }
  var rowS='display:flex;align-items:center;gap:8px;padding:8px 10px;border-bottom:0.5px dashed #e0e0e0;cursor:pointer;border-radius:6px;';
  var html='<div style="font-size:10px;color:#666;margin-bottom:6px;font-weight:700;">'+items.length+' &#1514;&#1493;&#1510;&#1488;&#1493;&#1514;</div>';
  // Standards & prices: show just title (and code) — no scope/desc text
  var isStd=(rowType==='standards'||rowType==='standard');
  var isPrice=(rowType==='prices'||rowType==='price');
  items.slice(0,500).forEach(function(i){
    var title=i.title||i.full_name||i.project_name||i.file_name||i.item_code||'&#8212;';
    var sub='';
    if(isStd){sub=i.standard_id||'';}
    else if(isPrice){sub=(i.item_code||'')+(i.price?' &#8212; '+i.price+' &#8362;':'');}
    else{sub=encFmtDate(i.created_at||i.archived_at||i.takeoff_date||'');}
    html+='<div style="'+rowS+'" onclick="encPreviewItem(\''+String(i.id).replace(/\'/g,"\\'")+'\')" onmouseover="this.style.background=\'#f5f0e8\'" onmouseout="this.style.background=\'\'">'+
      '<div style="flex:1;min-width:0;">'+
        ...
        (sub?'<div style="font-size:10px;color:#888;">'+encEsc(sub)+'</div>':'')+
      '</div>'+
      '<div style="font-size:11px;color:#9a6f00;">&#128065;&#65039;</div>'+
    '</div>';
  });
  if(items.length>500)html+='<div style="text-align:center;padding:10px;color:#888;font-size:11px;">&#1502;&#1510;&#1497;&#1490; 500 &#1502;&#1514;&#1493;&#1498; '+items.length+' &#8212; &#1505;&#1504;&#1503; &#1500;&#1510;&#1502;&#1510;&#1493;&#1501;</div>';
  return html;
}
...
function encPreviewItem(id){
  ...
  // Media types with a URL → use native preview
  if(url){
    if(type==='image'||type==='jpg'||type==='jpeg'||type==='png'||type==='heic'){
      // Self-contained image viewer — does NOT rely on host's openLightbox (DOM element may be missing)
    ...
    if(type==='audio'||type==='call'||type==='video'||type==='mp4'||type==='mov'){
      // Build inline audio/video player modal
    ...
    // Unknown type but has URL → just open
    window.open(url,'_blank');return;
  }

  // No URL → use existing encView detail modal (for standards, findings, notes, contacts, etc.)
